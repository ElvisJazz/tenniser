...
class PublicAction extends CommonAction{
        ...
        if($this->isPost()){
            if(session('verify') != md5($_POST['verify'])) {
                ...
                if($result) {
                    ...
                    $this->success('注册成功，请重新登录！', '__APP__/Index/index?displayInstruction=1');                    
                }else {
                    $this->error('注册失败！');
                }
            }
    ...
    public function logOnSubmit() {
        ...
        // 检测邮箱格式，方式SQL注入
        $target = "\"";
        $pos1 = strpos($_POST['mailbox'], $target); 
        $target = '\'';
        $pos2 = strpos($_POST['mailbox'], $target); 
        if(($pos1!=null || $pos2!=null) && ($pos1>=0 || $pos2>=0))
            $this->error('登录邮箱包含不合法字符，登录失败！');
        ...
        if($user!= null && $user[password] == $_POST['password'])
        {
            ...
            $this->success('登录成功！', '__APP__/Index/index_'.$club_id); 
        } else {
    ...
    public function userInfoSubmit() {
        ...
        if($result === FALSE) 
            ...
            cookie("tennis-user", null);
            $this->success('修改成功，请重新登录！', '__APP__');              
        }
    }
    ...
    public function logout() {
        ...
        $this->success('注销成功！', '__APP__/Index/index_'.$this->getClubId());
    }
       ...
       $pageId1;
        ...
        if($list != null){
            ...
            foreach($list as $key=>$value){
                // 设置头像
                if($value['portrait'] == 1){
                    $list[$key]['img'] =  $s->getUrl( 'imgdomain' , 'portrait/'.$value['id'].'.jpg');     
                } else{
